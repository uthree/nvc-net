...
class ASREncoder(nn.Module):
    def __init__(self, input_channels=4, output_channels=256):
        super().__init__()
        self.conv = nn.Conv1d(input_channels, output_channels, 1, 1, 0)
...
class ASRDecoder(nn.Module):
    def __init__(self, dim=256, num_layers=2, nhead=4, vocab_size=96):
        super().__init__()
    ...
    def forward(self, tgt, mem, tgt_mask=None, tgt_key_padding_mask=None):
        tgt = self.embedding(tgt)
        tgt = self.pos_emb(tgt)
        tgt = self.decoder(tgt, mem, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask)
        tgt = self.linear(tgt)
        return tgt
